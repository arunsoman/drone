...
def elapsed_micros(start_time):
    return (time.time() * 1E6 ) - start_time
...
class Fusion(object):
    ...
    def __init__(self, read_coro):
        ...
        self.beta = sqrt(3.0 / 4.0) * GyroMeasError *15 # compute beta (see README)
        self.pitch = 0
        self.heading = 0
        self.roll = 0
    ...
    def calibrate(self, stopfunc):
        _, _, mag = yield from self.read_coro()
        magmax = list(mag)                  # Initialise max and min lists with current values
        magmin = magmax[:]
        while not stopfunc():
            magxyz, _, _  = yield from self.read_coro()
            for x in range(3):
    ...
    def start(self, slow_platform=False):
        data = yield from self.read_coro()
        loop = asyncio.get_event_loop()
        if len(data) == 2:
            loop.create_task(self._update_nomag(slow_platform))
            print("No compass task.....")
        else:
            loop.create_task(self._update_mag(slow_platform))
        print("end of start")

    @asyncio.coroutine
    def _update_nomag(self, slow_platform):
        while True:
            accel, gyro = yield from self.read_coro()
    ...
    @asyncio.coroutine
    def _update_mag(self, slow_platform):
        while True:
            asyncio.sleep(0)
            accel, gyro, mag = yield from self.read_coro()
